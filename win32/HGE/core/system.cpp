...
wchar_t* CALL HGE_Impl::System_MakePath(const wchar_t *filename)
{
    int i;
...
void CALL HGE_Impl::System_SetStateString(hgeStringState state, const wchar_t *value)
{
    ...
    switch(state)
    {
        case HGE_ICON:          szIcon=value;
                                if(pHGE->hwnd) SetClassLong(pHGE->hwnd, GCL_HICON, (LONG)LoadIconW(pHGE->hInstance, szIcon));
                                break;
        case HGE_TITLE:         wcscpy(szWinTitle,value);
                                if(pHGE->hwnd) SetWindowTextW(pHGE->hwnd, szWinTitle);
                                break;
        case HGE_INIFILE:
                                break;
        case HGE_LOGFILE:       if(value)
                                {
                                    wcscpy(szLogFile,System_MakePath(value));
                                    hf=_wfopen(szLogFile, L"w");
                                    if(!hf) szLogFile[0]=0;
                                    else fclose(hf);
                                }
                                else szLogFile[0]=0;
...
bool CALL HGE_Impl::System_GetStateBool(hgeBoolState state)
{
    switch(state)
    {
        ...
        case HGE_USESOUND:      return false;
...
int CALL HGE_Impl::System_GetStateInt(hgeIntState state)
{
    switch(state)
    {
        ...
        case HGE_SAMPLERATE:    return 0;
        case HGE_FXVOLUME:      return 0;
        case HGE_MUSVOLUME:     return 0;
        case HGE_STREAMVOLUME:  return 0;
        case HGE_FPS:           return nHGEFPS;
        case HGE_POWERSTATUS:   return HGEPWR_UNSUPPORTED;
    }
...
{
    if((DWORD)ShellExecuteW(pHGE->hwnd, NULL, url, NULL, NULL, SW_SHOWMAXIMIZED)>32) return true;
    else return false;
}
...
void CALL HGE_Impl::System_Snapshot(const char *filename)
{
    ...
    if(pD3DDevice)
    {
        ...
        D3DXSaveSurfaceToFileW(filename, D3DXIFF_BMP, pSurf, NULL, NULL);
        pSurf->Release();
    }
}
#endif
...
HGE_Impl::HGE_Impl()
{
    ...
    GetModuleFileNameW(GetModuleHandle(NULL), szAppPath, sizeof(szAppPath));
    int i;
