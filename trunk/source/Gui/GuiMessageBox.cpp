...
int GuiMessageBox::DoSelection(void)
{
    ...
    // Run GUI
    GuiButton *selected = (GuiButton *)( Run() );
...
void GuiMessageBox::CleanUp(void)
{
    if( txt_sprite ) {
...
void GuiMessageBox::ShowPopup(GuiContainer *cntr, Image *image, int alpha, int delay, GuiEffect *effa, GuiEffect *effb, const char *txt, ...)
{
    va_list marker;
    va_start(marker, txt);

    GuiMessageBox *msgbox = new GuiMessageBox(cntr);
...
MSGBTN GuiMessageBox::ShowModal(GuiContainer *cntr, MSGT type, Image *image, int alpha, GuiEffect *effa, GuiEffect *effb, const char *txt, ...)
{
    va_list marker;
    va_start(marker, txt);
...
void GuiMessageBox::Create(MSGT type, Image *image, int alpha, const char *txt, ...)
{
    va_list marker;
    va_start(marker, txt);
...
void GuiMessageBox::CreateVA(MSGT type, Image *image, int alpha, const char *txt, va_list valist)
{
    CleanUp();
    ...
    SetWidth(sizex);
    SetHeight(sizey);
    frame = new GuiFrame(0, 0, sizex, sizey, alpha);
...
GuiMessageBox::GuiMessageBox(GuiContainer *cntr)
              :GuiDialog(cntr)
{
