...
void GuiCheckList::InitTitleList(TextRender *fontArial,
                                     int x, int y, int width, int ypitch)
{
    // Fill titles
    for(int i = 0, yy = y; i < num_item_rows; i++) {
        DrawableImage *img = new DrawableImage;
        img->CreateImage((width - CHECKBOX_WIDTH + 3) & ~3, ((fontsize * 3) / 2) & ~3);
        ...
        titleTxtSprite[i]->SetPosition(x + xspacing + CHECKBOX_WIDTH + xspacing, yy + fontsize/5);
        manager->AddTop(titleTxtSprite[i], fade);
        CheckboxSprite[i] = new Sprite;
        CheckboxSprite[i]->SetPosition(x + xspacing, yy + fontsize/5);
        manager->AddTop(CheckboxSprite[i], fade);
        yy += ypitch;
    }
...
void GuiCheckList::RemoveTitleList(void)
{
    ...
    for(int i = 0; i < num_item_rows; i++) {
        ...
        manager->RemoveAndDelete(CheckboxSprite[i], NULL, fade);
    }
}

void GuiCheckList::ClearTitleList(void)
{
    current_index = -1;
}

void GuiCheckList::SetSelected(int index, int selected)
{
    // Claim UI
    manager->Lock();
    // Update dir info
    ...
    if( selected >= 0 ) {
        ...
        sprSelector->SetStretchWidth((float)(xsize - CHECKBOX_WIDTH - xspacing) / 4);
...
void GuiCheckList::ShowSelection(const char **items, bool *enabled, int num, int select, int fontsz, int ypitch,
...
int GuiCheckList::DoSelection(void)
{
    // Menu loop
    u64 scroll_time = 0;
    ...
    for(;;) {
        ...
        if( (buttons & (WPAD_BUTTON_A | WPAD_CLASSIC_BUTTON_A | WPAD_BUTTON_2)) &&
            ...
            enabled_list[index+selected] = !enabled_list[index+selected];
            CheckboxSprite[selected]->SetImage(enabled_list[index+selected]?g_imgCheckboxChecked:g_imgCheckbox);
            break;
        }
...
void GuiCheckList::RemoveSelection(void)
{
    if( !is_showing ) {
        return;
    }
...
bool GuiCheckList::IsShowing(void)
{
    return is_showing;
}
...
GuiCheckList::GuiCheckList(GuiManager *man, int rows)
{
    ...
    CheckboxSprite = new Sprite*[rows];
}
