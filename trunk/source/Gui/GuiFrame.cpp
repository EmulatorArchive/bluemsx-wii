...
GuiFrame::GuiFrame( int posx,  int posy, int width, int height, int alpha)
...
void GuiFrame::SetPosition( int posx,  int posy)
{
    TiledLayer::SetPosition(posx - TILE_SIZE, posy - TILE_SIZE);
}

int GuiFrame::GetWidth(void)
{
    return GetColumns() * TILE_SIZE;
}

int GuiFrame::GetHeight(void)
{
    return GetRows() * TILE_SIZE;
}

void GuiFrame::Render(void)
{
